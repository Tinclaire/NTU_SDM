  ...
  return (
    <div className="mt-6 flex justify-center">
      <div className="w-full max-w-2xl">
          ...
          <table className="w-full border-collapse text-center">
            ...
            <tbody>
              {display.map((enrollment) => (
                <tr
                  key={`${enrollment.studentId}-${enrollment.semester}`}
                  onClick={() =>
                    handleClick(enrollment.studentId, enrollment.semester)
                  }
                  className="cursor-pointer hover:bg-blue-50 transition"
                >
                  <td className="p-3 border-t">{enrollment.semester}</td>
                  <td className="p-3 border-t">
                    {enrollment.studentId} / {enrollment.name}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {display.length === 0 && (
          <p className="text-center text-gray-500 mt-4">No Entry</p>
        )}
      </div>
    </div>
  );
